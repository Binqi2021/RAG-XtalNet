...
def load_model_ckpt(model_path, ckpt_path, load_data=False, testing=True, data_root_override=None):
    with initialize_config_dir(str(model_path)):
        cfg = compose(config_name='hparams')
        
        # Override data root path if provided
        if data_root_override is not None and hasattr(cfg, 'data'):
            print(f"Overriding data root path to: {data_root_override}")
            cfg.data.root_path = data_root_override
